 ...
 *
 */
package com.synectiks.library.utils;
...
import com.synectiks.library.utils.ESEvent.EventType;
...
public class SynectiksJPARepo<T, ID extends Serializable>
    ...
    public void fireEvent(EventType type, T entity) {
        if (IUtils.isNull(env)) {
            env = LibraryApp.getBean(Environment.class);
        }
        if (IUtils.isNull(rest)) {
            rest = LibraryApp.getBean(RestTemplate.class);
        }
        if (IUtils.isNull(applicationProperties)) {
            applicationProperties = LibraryApp.getBean(ApplicationProperties.class);
        }
